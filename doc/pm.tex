...
Rtnname(x1 : type1, x2 : type2, ..., xk : typek)
  ... preconditions ...
  {
	...
	In the following sections, for some of the functions implemented the actual program code is shown but not for all functions. The code shown is for functions which may not be very obvious or which show a typical implementation pattern used. If you understand the code snippets shown, the rest of the code should be no problem to read.
...
c_u_in_ua(#u{} = U, #ua{id = Y}) ->
	...
	\subsection{create\_x\_in\_y}
		...
		\begin{lstlisting}[caption={create\_x\_in\_y/3} numbers=left]
...
create_x_in_y(G, X, Y) ->
  ...
  case digraph:add_vertex(G) of
	...
	\subsection{delete\_u/2 and similar functions}
		...
		\begin{lstlisting}[caption={delete\_u/2}, numbers=left]
...
delete_u(G, X) ->
    ...
    mnesia:delete({u, X}). \end{lstlisting}
		...
		\begin{lstlisting}[caption={delete\_x/2}, numbers=left]
delete_x(G, X) ->
	...
	\subsection{create\_arset/2 and delete\_atset/2}\label{create_ar_atset}
		...
		\begin{lstlisting}[caption={create\_arset/2}, numbers=left]
...
create_arset(ARset, AR_ids) ->
  case mnesia:read(arset, ARset, read) of
	...
	\subsection{create\_assign/3}
		...
		\begin{lstlisting}[caption={create\_assign/3}, numbers=left]
...
create_assign(G, X, Y) ->
  ...
  case digraph:add_edge(G, Vx, Vy, Y) of
	...
	\section{Policy Administration Point}
	
	The PAP implements the Administrative Routines as defined in the specs. An administrative routine consists mainly of a parameterized interface and a sequence of administrative command invocations. Administrative routines build upon administrative commands to define the protection capabilities of the PM model. The body of an administrative routine is executed as an atomic transaction—an error or lack of capabilities that causes any of the constituent commands to fail execution causes the entire routine to fail, producing the same effect as though none of the commands were ever executed.
	
	Administrative routines are executed as transactions on the database which guaranties the consistency of the data stored, but the digraph used is \emph{not} implemented as a transactional store. The digraph must therefor be rebuild on a failure!
	
	The PAP is implemented as a server in the module \lstinline|pm_pap|. To implement the administrative routines, there is no need to use a server pattern but it does allow some extra administrative control (blocking, accounting etc.) and prohibits administrative clients to directly execute administrative routines.  NB: \emph{the implementation of the administrative routines may change in the future from calling the PAP server to simple function calls.}
	
	\emph{TBD: Verifying the administrative access rights for executing administrative routines is not yet implemented.}
	
	\emph{TBD: Make the error handling even more consistent. On failure of a administrative routine or if the interface finds a problem, an \lstinline|\{error, Reason\}| is returned, with Reason either being \lstinline|\{badarg, [Args]\}| or \lstinline|badvalue|. But, in case an argument contains a list and the list is being checked for containing a particular type of record, an exception is raised instead of returning an badvalue error}. 
	
	An example of a typical implementation of a administrative routine in \lstinline|pm_pap|:
	
	\begin{lstlisting}[caption={c\_u\_in\_ua/2 interface}, numbers=left]
...
c_u_in_ua(#u{} = U, #ua{} = UA) ->
	gen_server:call(?SERVER, {c_u_in_ua, U, UA}).\end{lstlisting}

	The interface matches the parameters and/or uses guards to check for correct argument types and values. This is done in the process making the call so any errors due to incorect arguments are raised in the calling process. The administrative routine is implemented as a call in the server:
	
	\begin{lstlisting}[caption={c\_u\_in\_ua/2 administrative command invocations}, numbers=left]
handle_call({c_u_in_ua, U, UA}, _From, #state{g = G} = State) ->
	...
	{reply, Reply, State};\end{lstlisting}

	The routine is implemented as a \lstinline|fun()| which  will perform one or more administrative commands in the PIP and which is executed as a transaction. The digraph \lstinline|G| the routine acts on is kept in the PAP's state.
	In this example, the call to \lstinline|pm_pip:create_ua_in_ua/3| will return the tuple \lstinline|{ok, U}| with U being the user record as it was passed in to the routine with the new Id added to it and the user record being stored in the database. This way, a user record can contain whatever data is needed in the application and it being stored properly in the database, with only the id under control of the PM.
	
	Another, more complicated, example of an interface is the one for creating an association:
	
	\begin{lstlisting}[caption={c\_assoc/3 administrative routine interface}, numbers=left]
...
ATE_ids = [Id || #ua{id = Id}) <- ATEs]\end{lstlisting}
		 ...
		 but the first form not only extracts the Id from the record but also checks that each of the elements in the list ATEs is indeed a \lstinline|#ua{}| record. The second form will silently ignore records of a different type.
		 
		 \item For \lstinline|pm_pip:allocate_id/1| the \lstinline|erlang:phash2/2| function is used on the sorted list of valeus with duplicates removed. According to the documentation the \lstinline|erlang:phash2/2| will give the same result on the same term for every version and implementation of Erlang. Is this also true if the implementation of the list is changed?
		 
		 \item What is the proper way to handle error conditions. When to return an {error, Reason} tuple and when to raise an exception? A good rule of thumb is to use
		 exceptions for errors that are truly exceptional.  See \href{http://erlang.org/pipermail/erlang-questions/2005-August/016545.html}{this post}.
